use nirah_uri::*;
use nirah_sip::headers::parse::parse_to_header;
...
fn write() {
    ...
    let header = Header::To(NamedHeader::new(uri).name("Guy"));
    assert_eq!("To: Guy <sip:guy@example.com>".to_string(), format!("{}", header));

    let uri = Uri::sip(domain!("example.com")).auth(uri_auth!("guy"));
    let header = Header::To(NamedHeader::new(uri).name(""));
    assert_eq!("To:  <sip:guy@example.com>".to_string(), format!("{}", header));

    let uri = Uri::sip(domain!("example.com")).auth(uri_auth!("guy"));
    let header = Header::To(NamedHeader::new(uri).name("Guy With Face"));
    assert_eq!("To: \"Guy With Face\" <sip:guy@example.com>".to_string(), format!("{}", header));

    let uri = Uri::sip(domain!("example.com")).auth(uri_auth!("guy"));
    let header = Header::To(NamedHeader::new(uri));
    assert_eq!("To: <sip:guy@example.com>".to_string(), format!("{}", header));
}
