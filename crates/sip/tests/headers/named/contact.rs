use nirah_uri::*;
use nirah_sip::headers::parse::parse_contact_header;
...
fn write() {
    ...
    let header = Header::Contact(NamedHeader::new(uri).name("Guy"));
    assert_eq!("Contact: Guy <sip:guy@example.com>".to_string(), format!("{}", header));

    let uri = Uri::sip(domain!("example.com")).auth(uri_auth!("guy"));
    let header = Header::Contact(NamedHeader::new(uri).name("Guy With Face"));
    assert_eq!("Contact: \"Guy With Face\" <sip:guy@example.com>".to_string(), format!("{}", header));

    let uri = Uri::sip(domain!("example.com")).auth(uri_auth!("guy"));
    let header = Header::Contact(NamedHeader::new(uri));
