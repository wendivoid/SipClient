...
use nirah_sip::headers::parse::parse_reply_to_header;

#[test]
fn write() {
    let uri = Uri::sip(domain!("example.com")).auth(uri_auth!("guy"));
    let header = Header::ReplyTo(NamedHeader::new(uri).name("Guy"));
    assert_eq!("Reply-To: Guy <sip:guy@example.com>".to_string(), format!("{}", header));

    let uri = Uri::sip(domain!("example.com")).auth(uri_auth!("guy"));
    let header = Header::ReplyTo(NamedHeader::new(uri).name("Guy With Face"));
    assert_eq!("Reply-To: \"Guy With Face\" <sip:guy@example.com>".to_string(), format!("{}", header));

    let uri = Uri::sip(domain!("example.com")).auth(uri_auth!("guy"));
    let header = Header::ReplyTo(NamedHeader::new(uri));
